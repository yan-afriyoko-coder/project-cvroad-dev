...
dealer_experience-->
      ...
      <div class="update-news mb-5">        
        ...
        <div class="row">          
            ...
            <div class="col-md-2 job-filter">
              <button type="submit" class="btn btn-main form-control mt-2"> <i class="fas fa-search"></i> Search </button>
            </div>                    
        </div>    
      </form>    
      </div>
        ...
        <div class="team-person py-3">
          <div class="row">
            ...
            <div class="col-md-2 col-xs-6 mt-2">
              <div class="person" style="background-color: #d9fcea; padding:10px; border-radius:15%">
                <div class="photo">
                  @if($candidate->avatar)
                        <img src="{{asset('uploads/avatar/'. $candidate->avatar)}}"  style="max-width: 40%">
                      @else  
                        <img src="{{asset('uploads/avatar/account.png')}}"  alt="" style="max-width: 40%">
                @endif
                </div>
                
                {{-- <div class="photo"><img src="{{asset('assets/img/team-1.jpg')}}" alt="" style="max-width: 40%"></div> --}}
                <span class="name">{{$candidate->name}} {{$candidate->surname}}</span>
                <span> <strong>{{$candidate->title}}</strong></span>
                <small><strong>Group:</strong>{{$candidate->group->name ?? ''}} </small>
                <small><strong>Deprtment:</strong>{{$candidate->department->name ?? ''}} </small><br>
                <small> <i class="fas fa-phone"></i> : {{$candidate->phone_number}} </small><br>
                <a href="{{route('dealer_job_candidate',[$candidate->user->id])}}" class="btn btn-sm btn-main">profile</a>
              </div>            
            </div>              
            @endforeach
            @else
